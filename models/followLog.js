...
FollowLog.saveSearchFollow = (account, search_result) => {
  new Log({ direction: 0, follow_type: 0, account, search_result }).save().catch(logError)
}
FollowLog.saveFollowBack = (account) => {
  new Log({ direction: 0, follow_type: 1, account }).save().catch(logError)
}
FollowLog.saveUnfollow = (account) => {
  new Log({ direction: 0, follow_type: 2, account }).save().catch(logError)
}
FollowLog.saveFollowed = (account) => {
  new Log({ direction: 1, account }).save().catch(logError)
}
